    ...
    protected function _populateEvent( \BooklyGoogle_Service_Calendar_Event $event, Appointment $appointment )
    {
        ...
        if ( $appointment->getCreatedFrom() === 'bookly' || get_option( 'bookly_gc_force_update_description' ) ) {
            ...
            $appointment_info = strtr( get_option( 'bookly_gc_event_appointment_info' ), array(
                ...
                '{online_meeting_password}'  => BooklyLib\Proxy\Shared::buildOnlineMeetingPassword( '', $appointment ),
                '{online_meeting_start_url}' => BooklyLib\Proxy\Shared::buildOnlineMeetingStartUrl( '', $appointment ),
                ...
                '{on_waiting_list}'   => BooklyLib\Config::waitingListActive()
                    ? CustomerAppointment::query( 'ca' )
                        ...
                        ->where( 'status', CustomerAppointment::STATUS_WAITLISTED )
                        ->count()
                    : 0,
            ) );
            if ( $codes == null ) {
